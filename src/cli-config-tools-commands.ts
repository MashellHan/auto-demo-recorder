...
import { registerConfigAnalysisCommands } from './cli-config-analysis-commands.js';

/**
 * Register config tool CLI commands onto the given program.
 *
 * Split from cli-config-commands.ts to keep files under 400 lines.
 * Commands: lint, check, config-merge, graph, scaffold, diagnose, config-export.
 *
 * Analysis commands (clone, interpolate, config-compare, depth, complexity,
 * dep-health) delegated to cli-config-analysis-commands.ts.
 */
export function registerConfigToolCommands(program: Command): void {
  ...
  registerConfigAnalysisCommands(program);
}
